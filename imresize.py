...
import os
def test(filename, k=None):
    # filename = "d:/workroom/testroom/v0.png"
    print(filename)
    dir = "d:/workroom/tools/dataset/SR/tmp/"
    outfile = dir + os.path.basename(filename).replace(".png", "_lr.png")
    hr_img = cv2.imread(filename)
    print(hr_img.shape)
    hr_img = Image.open(filename)
    hr_img = TF.to_tensor(hr_img)

    HR_img = TF.to_pil_image(hr_img)
    print(type(HR_img))
    resize_img = imresize(np.array(HR_img), scale_factor=1.0 / 2.0, kernel=k)
    print(resize_img.shape)
    print(type(resize_img))
    resize_img = TF.to_pil_image(resize_img.astype(np.uint8))
